...
export const actions = {
    complain: async ({ request, locals }) => {
        ...
        if (razon.length < 3) {
            return fail(400, {
                message:
                    'La razón del reclamo debe tener al menos 3 caracteres.',
            })
        }
        ...
        if (!tarjeta) {
            return fail(404, {
                message: 'La tarjeta no existe o no te pertenece.',
            })
        }
        ...
        if (existingComplaint) {
            return fail(409, {
                message: 'Esta tarjeta ya fue reclamada una vez.',
            })
        }
        ...
        await prisma.reclamosCarta.create({
            data: {
                ...
                razon,
                fechaReclamo:
                    parseUTCDate(new Date().toISOString()) ?? new Date(),
            },
        })
