...
export const actions = {
    ...
    set_positions: async ({ request, locals, cookies }) => {
        ...
        const positions = POSITION_OPTIONS.map((_, index) => {
            return (
                (formData.get(`position_${index + 1}`) as string | null) ?? ''
            ).trim()
        })
        ...
        const hasUniquePositions =
            new Set(positions).size === POSITION_OPTIONS.length
    ...
    set_cumpleanos: async ({ request, locals, cookies }) => {
        ...
        if (candidate.getTime() < minDate.getTime()) {
            return fail(400, {
                success: false,
                message:
                    'La fecha de cumpleaños no puede ser anterior a hace 120 años',
            })
        }
    ...
    set_password: async ({ request, locals, cookies }) => {
        ...
        const new_password =
            (formData.get('new_password') as string | null) ?? ''
