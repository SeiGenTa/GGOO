                            ...
                            <form
                                ...
                                use:enhance={() => {
                                    return async ({ result, update }) => {
                                        if (result.type === "success") {
                                            const message =
                                                (result.data as { message?: string } | null)?.message ?? "Rol predeterminado actualizado.";
                                            toast("Éxito", { description: message });
                                            await update();
                                            return;
                                        }
                                        ...
                                        if (result.type === "failure") {
                                            const message =
                                                ...
                                                "No fue posible actualizar el rol predeterminado.";
                                            toast("Error", { description: message });
                                            await update();
                                            return;
                                        }
                        ...
                        <form method="POST" action="?/update_role" class="space-y-4" use:enhance={withFeedback("Rol actualizado")}>
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="grid gap-2 sm:grid-cols-2">
                                    {#each data.permissions as permission}
                                        <label class="flex items-center gap-2 rounded-md border p-2 text-sm">
                                            <input type="checkbox" name="permisos" value={permission} checked={role.permisos.includes(permission)} />
                                            <span>{getPermissionLabel(permission)}</span>
                                        </label>
                                    {/each}
                                </div>
