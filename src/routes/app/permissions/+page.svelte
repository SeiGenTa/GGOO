<script lang="ts">
    ...
    const toggleDefaultRole = (roleId: string) => {
        const form = document.getElementById(`default-role-form-${roleId}`) as HTMLFormElement | null;
        if (form) {
            form.requestSubmit();
        }
    };
    ...
    {#if data.blocked}
        <Card.Root>
            <Card.Header>
                ...
                <Card.Description>Necesitas permisos de administración de roles para acceder a esta sección.</Card.Description>
            </Card.Header>
        </Card.Root>
    {:else}
            ...
            <Card.Header>
                <div class="flex items-start justify-between gap-3">
                    <div>
                        ...
                        <Card.Description>Administra los roles del sistema desde tarjetas adaptadas a móvil.</Card.Description>
                    </div>
                    <Button type="button" variant="outline" onclick={openCreateDialog}>Crear rol</Button>
                </div>
            </Card.Header>
        </Card.Root>
            ...
            {#each data.roles as role}
                <Card.Root class="shadow-sm">
                    <Card.Header>
                        <div class="flex items-start justify-between gap-3">
                            <div class="min-w-0">
                                <Card.Title class="truncate">{role.nombre}</Card.Title>
                                <Card.Description>
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <form
                                    ...
                                    method="POST"
                                    use:enhance={withFeedback("Rol predeterminado actualizado")}
                                >
                                    <input type="hidden" name="roleId" value={role.id} />
                                    <input type="hidden" name="is_default" value={role.is_default ? "false" : "true"} />
                                    <Switch checked={role.is_default} onCheckedChange={() => toggleDefaultRole(role.id)} />
                                </form>
                            </div>
                        </div>
                    </Card.Header>
                        ...
                        <div class="flex flex-wrap gap-1.5">
                            ...
                            {:else}
                                {#each role.permisos as permission}
                                    <Badge variant="secondary">{getPermissionLabel(permission)}</Badge>
                                {/each}
                            {/if}
                        </div>

                        <div class="flex flex-wrap gap-2">
                            <Button type="button" variant="outline" size="sm" onclick={() => openEditDialog(role)}>Editar</Button>
                            <form method="POST" action="?/delete_role" use:enhance={withFeedback("Rol eliminado")}>
                                <input type="hidden" name="roleId" value={role.id} />
...
<Dialog.Root bind:open={createDialogOpen}>
    <Dialog.Content class="flex max-h-[90dvh] flex-col overflow-hidden">
        <Dialog.Header>
            ...
            <Dialog.Description>Define un nombre y selecciona los permisos que tendrá este rol.</Dialog.Description>
        </Dialog.Header>

        <form method="POST" action="?/create_role" id="create-role-form" class="contents" use:enhance={withFeedback("Rol creado")}>
                    ...
                    <div class="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                        {#each data.permissions as permission}
                            <label class="flex items-center gap-2 rounded-md border p-2 text-sm">
                                <input type="checkbox" name="permisos" value={permission} bind:group={createRolePermissions} />
...
<Dialog.Root bind:open={editDialogOpen}>
    <Dialog.Content class="flex max-h-[90dvh] flex-col overflow-hidden">
        ...
        {#if selectedRole}
            <form method="POST" action="?/update_role" id="edit-role-form" class="contents" use:enhance={withFeedback("Rol actualizado")}>
                <input type="hidden" name="roleId" value={selectedRole.id} />

                <div class="min-h-0 flex-1 space-y-4 overflow-y-auto py-2 pr-1">
                    <div class="space-y-2">
                        <label class="text-sm font-medium" for={`name-${selectedRole.id}`}>Nombre</label>
                        <Input id={`name-${selectedRole.id}`} name="nombre" bind:value={editRoleName} required />
                    </div>
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                            {#each data.permissions as permission}
                                <label class="flex items-center gap-2 rounded-md border p-2 text-sm">
                                    <input type="checkbox" name="permisos" value={permission} bind:group={editRolePermissions} />
                                    <span>{getPermissionLabel(permission)}</span>
                                </label>
                            {/each}
                        </div>
