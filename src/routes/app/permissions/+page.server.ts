...
const parsePermissions = (form: FormData) => {
    ...
    return selected.filter((permission) =>
        ALL_PERMISSIONS.includes(permission as Permissions)
    )
}
...
export const actions: Actions = {
    create_role: async ({ request, locals }) => {
        if (!locals.user) {
            logger.info(
                { action: 'action_create_role_unauthorized' },
                'Intento de crear rol sin autenticación'
            )
        ...
        if (!locals.user.permisos.includes(Permissions.CrearRoles)) {
            logger.info(
                ...
                'Intento de crear rol sin permisos'
            )
            return fail(403, {
    ...
    update_role: async ({ request, locals }) => {
        if (!locals.user) {
            logger.info(
                { action: 'action_update_role_unauthorized' },
                'Intento de editar rol sin autenticación'
            )
            return fail(401, { message: 'No autorizado.' })
        }
        ...
        if (!locals.user.permisos.includes(Permissions.EditarRoles)) {
            logger.info(
                ...
                'Intento de editar rol sin permisos'
            )
            return fail(403, {
        ...
        logger.info(
            `Rol ${nombre} actualizado por el usuario ${locals.user.id}`
        )
    ...
    delete_role: async ({ request, locals }) => {
        if (!locals.user) {
            logger.info(
                { action: 'action_delete_role_unauthorized' },
                'Intento de eliminar rol sin autenticación'
            )
            return fail(401, { message: 'No autorizado.' })
        }
        ...
        if (!locals.user.permisos.includes(Permissions.EliminarRoles)) {
            logger.info(
                ...
                'Intento de eliminar rol sin permisos'
            )
            return fail(403, {
    ...
    set_predeterminated: async ({ request, locals }) => {
        if (!locals.user) {
            logger.info(
                { action: 'action_set_default_role_unauthorized' },
                'Intento de actualizar rol predeterminado sin autenticación'
            )
            return fail(401, { message: 'No autorizado.' })
        }
        ...
        if (!locals.user.permisos.includes(Permissions.EditarRoles)) {
            logger.info(
                ...
                'Intento de actualizar rol predeterminado sin permisos'
            )
            return fail(403, {
                message: 'No tienes permisos para editar roles.',
            })
        ...
        logger.info(
            `Rol ${roleId} actualizado como predeterminado=${isDefault} por el usuario ${locals.user.id}`
        )
