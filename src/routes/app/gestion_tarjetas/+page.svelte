<script lang="ts">
    ...
    const matchUserSearch = (
        user: { apodo: string | null; nombre: string; email: string },
        search: string
    ) => {
        ...
        return label.includes(term) || user.email.toLowerCase().includes(term)
    }
    ...
    const personaOptions = $derived(
        buildUserOptions(personaSearch, selectedUserIds)
    )

    const asignadoPorOptions = $derived(
        buildUserOptions(asignadoPorSearch, selectedAssignedByIds)
    )
    ...
    const complaintStatusLabel = (card: TarjetaRow) => {
        if (card.complaint) {
            return card.complaint.atendido
                ? 'Reclamo resuelto'
                : 'Reclamo pendiente'
        }
...
<div class="container mx-auto space-y-6 p-4">
        ...
        <Card.Header>
            <div class="flex items-start justify-between gap-3">
                <div>
                    ...
                    <Card.Title class="mt-1 text-3xl"
                        >Gestión de tarjetas</Card.Title
                    >
                </div>
                <Button
                    type="button"
    ...
    <Card.Root>
        <Card.Header>
            ...
            <Card.Description
                >Busca por texto, persona, asignador, tipo y estado de uso.</Card.Description
            >
        </Card.Header>
        <Card.Content class="space-y-4">
            ...
            >
                <div class="lg:col-span-2">
                    <label class="mb-1 block text-sm font-medium" for="q"
                        >Buscar</label
                    >
                    <Input
                        id="q"
                ...
                <div>
                    <label class="mb-1 block text-sm font-medium" for="tipo"
                        >Tipo</label
                    >
                    <select
                        id="tipo"
                    ...
                    >
                        <option
                            value=""
                            selected={(data.filters?.tipo ?? '') === ''}
                            >Todos</option
                        >
                        {#each data.cardTypes as tipo}
                            <option
                                value={tipo}
                                selected={(data.filters?.tipo ?? '') === tipo}
                                >{tipo}</option
                            >
                        {/each}
                    </select>
                ...
                <div>
                    <label class="mb-1 block text-sm font-medium" for="usado"
                        >Estado</label
                    >
                    <select
                        id="usado"
                    ...
                    >
                        <option
                            value=""
                            selected={(data.filters?.usado ?? '') === ''}
                            >Todos</option
                        >
                        <option
                            value="false"
                            selected={(data.filters?.usado ?? '') === 'false'}
                            >Vigente</option
                        >
                        <option
                            value="true"
                            selected={(data.filters?.usado ?? '') === 'true'}
                            >Usada/Cancelada</option
                        >
                    </select>
                </div>
                ...
                <div>
                    <label
                        ...
                        for="venceEnDesde">Vence desde (UTC)</label
                    >
                    <Input
                        id="venceEnDesde"
                        name="venceEnDesde"
                        type="datetime-local"
                        value={data.filters?.venceEnDesde
                            ? toDateTimeLocalInput(data.filters.venceEnDesde)
                            : ''}
                    />
                </div>
                ...
                <div>
                    <label
                        class="mb-1 block text-sm font-medium"
                        for="venceEnHasta">Vence hasta (UTC)</label
                    >
                    <Input
                        id="venceEnHasta"
                        name="venceEnHasta"
                        type="datetime-local"
                        value={data.filters?.venceEnHasta
                            ? toDateTimeLocalInput(data.filters.venceEnHasta)
                            : ''}
                    />
                </div>
                ...
                <div>
                    <label class="mb-1 block text-sm font-medium" for="orderBy"
                        >Ordenar por</label
                    >
                    <select
                        id="orderBy"
                    ...
                    >
                        <option
                            value="createdAt"
                            selected={(data.filters?.orderBy ?? 'createdAt') ===
                                'createdAt'}>Fecha</option
                        >
                        <option
                            value="venceEn"
                            selected={(data.filters?.orderBy ?? 'createdAt') ===
                                'venceEn'}>Vencimiento</option
                        >
                        <option
                            value="tipoCarta"
                            selected={(data.filters?.orderBy ?? 'createdAt') ===
                                'tipoCarta'}>Tipo</option
                        >
                        <option
                            value="usado"
                            selected={(data.filters?.orderBy ?? 'createdAt') ===
                ...
                <div>
                    <label class="mb-1 block text-sm font-medium" for="orderDir"
                        >Dirección</label
                    >
                    <select
                        id="orderDir"
                    ...
                    >
                        <option
                            value="desc"
                            selected={(data.filters?.orderDir ?? 'desc') ===
                ...
                <div class="lg:col-span-4 flex flex-wrap gap-2">
                    ...
                    <Button href="/app/gestion_tarjetas" variant="outline"
                        >Limpiar</Button
                    >
                            ...
                            <div class="flex items-start justify-between gap-3">
                                <div class="min-w-0">
                                    ...
                                    <p
                                        class="truncate text-xs text-muted-foreground"
                                    >
                                        Asignada por {tarjeta.asignadoPor}
                                    </p>
                                </div>
                                <Badge
                            ...
                            <div class="mt-4 space-y-3">
                                ...
                                >
                                    <p>
                                        ...
                                        {formatDate(tarjeta.fechaCreacion)}
                                    </p>
                                    <p>
                                ...
                                <div class="flex flex-wrap gap-2">
                                    ...
                                    {#if tarjeta.complaint}
                                        <Badge
                                            variant={tarjeta.complaint.atendido
                                                ? 'secondary'
                                                : 'default'}
                                        >
                                            {tarjeta.complaint.atendido
                                                ? 'Reclamo resuelto'
                                                : 'Reclamo pendiente'}
                                        </Badge>
                                    {:else}
                                    ...
                                    {#if data.canEdit}
                                        ...
                                        >
                                            ...
                                            <div class="grid grid-cols-2 gap-2">
                                                <select
                                                    name="tipo"
                                                    class="h-10 rounded-md border bg-background px-2 text-xs"
                                                >
                                                    {#each data.cardTypes as tipo}
                                                        <option
                                                            value={tipo}
                                                            selected={tarjeta.tipo ===
                                                                tipo}
                                                            >{tipo}</option
                                                        >
                                                    {/each}
                    ...
                    <tbody>
                        {#if data.tarjetas.length === 0}
                            <tr>
                                ...
                                >
                                    No se encontraron tarjetas con los filtros
                                    actuales.
                                </td>
                            </tr>
                        {:else}
                            {#each data.tarjetas as tarjeta}
                                <tr class="border-b align-top">
                                    ...
                                    <td
                                        class="max-w-80 px-4 py-3 text-xs leading-relaxed text-muted-foreground"
                                        >{tarjeta.razon}</td
                                    >
                                    <td class="px-4 py-3 text-xs"
                                        >{formatDate(tarjeta.fechaCreacion)}</td
                                    >
                                    <td class="px-4 py-3 text-xs"
                                    ...
                                    <td class="px-4 py-3">
                                        {#if tarjeta.complaint}
                                            <div class="space-y-1">
                                                ...
                                                <p
                                                    class="max-w-56 text-xs text-muted-foreground"
                                                >
                                                    {tarjeta.complaint.razon}
                                                </p>
            ...
            >
                <p class="text-muted-foreground">
                    Mostrando página {data.pagination?.page ?? 1} de {data
                        .pagination?.totalPages ?? 1}. Total tarjetas: {data
...
<Dialog.Root bind:open={isCreateDialogOpen}>
    <Dialog.Content>
        <Dialog.Header>
            ...
            <Dialog.Description>
                Completa los parámetros para asignar una nueva tarjeta a un
                usuario.
            </Dialog.Description>
        </Dialog.Header>
        ...
        <form
            ...
            id="create-card-form"
            class="space-y-3"
            use:enhance={withUTCDateFields(['venceEn'], 'Tarjeta creada', true)}
        >
            <div class="space-y-1">
                <label class="block text-sm font-medium" for="id_personas"
                    >Persona</label
                >
                <Input
            ...
            <div class="space-y-1">
                <label class="block text-sm font-medium" for="tipo-create"
            ...
            <div class="space-y-1">
                <label class="block text-sm font-medium" for="venceEn-create"
                    >Vence en (UTC)</label
                >
                <Input
                    id="venceEn-create"
                ...
                <p class="text-xs text-muted-foreground">
                    Si no indicas fecha, se asignará automáticamente un mes
                    desde hoy.
                </p>
            </div>

            <div class="space-y-1">
                <label class="block text-sm font-medium" for="razon"
                    >Razón</label
                >
                <textarea
                    id="razon"
    ...
    <Dialog.Content>
        <Dialog.Header>
            ...
            <Dialog.Description>
                {#if selectedComplaint}
                    Responde al reclamo y decide si se rechaza o si se cancela
                    la tarjeta.
                {:else}
        ...
        {#if selectedComplaint}
            <form
                ...
                id="resolve-complaint-form"
                class="space-y-4"
                use:enhance={withFeedback('Reclamo actualizado')}
            >
                <input
                    type="hidden"
                    name="complaintId"
                ...
                <div class="space-y-1">
                    <label
                        ...
                        >Respuesta del administrador</label
                    >
                    <textarea
...
<Dialog.Root bind:open={helpDialogOpen}>
    <Dialog.Content>
        <Dialog.Header>
            ...
            <Dialog.Description
                >Resumen rápido de la gestión administrativa de tarjetas.</Dialog.Description
            >
        </Dialog.Header>

        <div class="space-y-3 text-sm text-muted-foreground">
            <p>
                Desde esta vista puedes crear, editar y eliminar tarjetas,
                además de revisar los reclamos de los usuarios.
            </p>
            <p>
                Cuando una tarjeta tenga un reclamo pendiente, podrás leerlo,
                responderlo y decidir si se rechaza o si la tarjeta se cancela
                como usada.
            </p>
            <p>
                También puedes revisar si una tarjeta ya fue usada o venció,
                para mantener el control disciplinario.
            </p>
        </div>
