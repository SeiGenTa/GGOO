...
const getValidPayload = async (token: string) => {
    ...
    if (!decoded?.email || decoded.reason !== RECOVER_REASON || !decoded.password) {
        return null;
    }
...
export const load: PageServerLoad = async ({ url }) => {
    ...
    if (!validated) {
        throw redirect(303, "/auth?error=Token%20de%20recuperacion%20invalido%20o%20expirado.");
    }
...
export const actions = {
    default: async ({ request }) => {
        ...
        logger.info(
            { action: "action_recover_password_changed", userId: validated.user.id, email: validated.user.email },
