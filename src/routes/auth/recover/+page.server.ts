...
export const actions = {
    default: async ({ request }) => {
        ...
        if (!user) {
            logger.info(
                { action: 'action_recover_password_user_not_found', email },
                'Solicitud de recuperacion para correo inexistente'
            )
            return {
                success: true,
            }
        ...
        logger.info(
            {
                action: 'action_recover_password_email_sent',
                userId: user.id,
                email: user.email,
            },
            'Correo de recuperacion de clave enviado'
        )
